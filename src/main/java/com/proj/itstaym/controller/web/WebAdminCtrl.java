...
public class WebAdminCtrl {
    ...
    public String showLendingReport(@AuthenticationPrincipal UserDetails userDetails, Model model) {
        ...
        if (role.equals("ADMIN")) {
            model.addAttribute("pageLabel", "Admin");
            model.addAttribute("contentFragment", "fragments/pages/admin/borrow_return_history");
            model.addAttribute("cssFiles", List.of("/css/admin/borrow.return.history.css"));
            model.addAttribute("jsFiles", List.of("/js/admin/borrow.return.history.js"));
        } else {
            model.addAttribute("contentFragment", "fragments/error/unauthorized");
            model.addAttribute("status", "403");
            model.addAttribute("error", "Authority Not Verified");
